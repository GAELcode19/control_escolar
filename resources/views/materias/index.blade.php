...
<div class="flex flex-col gap-6 p-6">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <div class="bg-[#1e1e2e] border border-white/10 rounded-xl p-4 flex items-center gap-4">
            ...
            <div>
                ...
                <p class="text-2xl font-bold text-white">{{ $horasTotales ?? 0 }}</p>
            </div>
        </div>
    </div>
    ...
    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
        ...
        <a href="#" class="flex items-center gap-2 bg-violet-600 hover:bg-violet-500 transition text-white text-sm font-medium px-4 py-2 rounded-lg">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        <div class="bg-[#1e1e2e] border border-white/10 rounded-xl p-5 flex flex-col gap-4 hover:border-violet-500/40 transition group">
            <div class="flex items-start justify-between">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <p class="text-white font-semibold text-sm">{{ $materia->nombre }}</p>
                        <p class="text-gray-400 text-xs">Clave: {{ $materia->clave ?? '—' }}</p>
                    </div>
                </div>
                @if(($materia->estatus ?? 'Activa') === 'Activa')
            ...
            <div class="grid grid-cols-2 gap-2 text-xs">
                <div class="bg-white/5 rounded-lg px-3 py-2">
                    ...
                    <p class="text-white font-semibold">{{ $materia->creditos ?? '—' }}</p>
                </div>
                <div class="bg-white/5 rounded-lg px-3 py-2">
                    <p class="text-gray-400">Horas/sem</p>
                    <p class="text-white font-semibold">{{ $materia->horas_semana ?? '—' }}</p>
                </div>
            </div>

            <div class="flex items-center justify-between pt-1 border-t border-white/5">
                <p class="text-xs text-gray-500">{{ $materia->descripcion ?? 'Sin descripción' }}</p>
                <div class="flex gap-1 opacity-0 group-hover:opacity-100 transition">
                    <a href="#" class="p-1.5 rounded-lg hover:bg-violet-500/20 text-gray-400 hover:text-violet-400 transition">
                        <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                        </svg>
                    </a>
                    <a href="#" class="p-1.5 rounded-lg hover:bg-red-500/20 text-gray-400 hover:text-red-400 transition">
                        <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div class="col-span-3 py-16 text-center">
            <div class="flex flex-col items-center gap-3 text-gray-500">
                ...
                <a href="#" class="text-violet-400 hover:underline text-xs">+ Agregar la primera</a>
            </div>
        </div>
        @endforelse
    </div>
